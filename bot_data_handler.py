...
class BotDataHandler:
    ...
    VACCINE_TYPES = ['Covishield', 'Covaxin', 'Sputnik V', 'All vaccine']
    ...
    def add_user(self, user, user_data):
        ...
        self.db_session.merge(User(user_id=user.id, uname=uname, fname=fname,
                                   ...
                                   age_group=age, is_subscribed=True))
    ...
    def resume_update_for_user(self, user_id):
        user = self.db_session.query(User).get(user_id)
        if (user is not None) and not user.is_subscribed:
    ...
    def get_vaccine_centers_for_user(self, user_id):
        ...
        if centers is not None and len(centers) == 0:
            ...
            no_vaccine_msg = 'Currently no vaccination slots are available in {} for age group {}'.format(
                self.get_area_str(area_code, area_type), self.get_age_str2(age))
    ...
    def get_filtered_data_for_location(self, age_groups, area_code, is_pincode, slot_threshold=1):
        ...
        for age_grp in age_groups:
            centers = list(CowinCenter.build_and_get_filtered_centers(api_data.get("centers", []),
                                                                      age_grp,
                                                                      slot_threshold))
    ...
    def get_area_update_record(self, area_type, area_code, age_gp):
        age_gp = int(age_gp)
        area_rec = self.db_session.query(AreaUpdate).filter_by(area_type=area_type, area_code=area_code,
                                                               age_gp=age_gp).first()
        if not area_rec:
